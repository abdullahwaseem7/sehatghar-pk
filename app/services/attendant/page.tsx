...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "https://mysehatconnect.com/services/attendant" },
};
  ...
  return (
    <>
      ...
      <main style={{ backgroundColor: "#F5FAF9", minHeight: "80vh" }}>
        <div style={{ backgroundColor: "#0A5E55", padding: "48px 0" }}>
          <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8">
            <span style={{ backgroundColor: "rgba(255,255,255,0.2)", color: "#fff", fontSize: "11px", fontWeight: 600, padding: "4px 12px", borderRadius: "20px" }}>Trained & Verified</span>
            <h1 style={{ fontSize: "clamp(24px, 4vw, 36px)", fontWeight: 700, color: "#fff", marginTop: "12px", marginBottom: "10px" }}>Attendant</h1>
            <p style={{ fontSize: "15px", color: "#B2DED9", marginBottom: "16px" }}>Non-clinical personal care and companionship from trained, background-verified attendants.</p>
            <div style={{ fontSize: "26px", fontWeight: 700, color: "#fff" }}>Rs. {PRICES.attendant.toLocaleString()} <span style={{ fontSize: "14px", fontWeight: 400, color: "#B2DED9" }}>/ 12-hour shift</span></div>
          </div>
        </div>

        <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
            {ATTENDANT_SERVICES.map(s => (
              <div key={s.id} style={{ backgroundColor: "#fff", border: "0.5px solid #B2DED9", borderRadius: "12px", padding: "20px" }}>
                ...
                <Link href={`/book?service=${s.id}&category=attendant`}
                  style={{ fontSize: "12px", fontWeight: 600, color: "#0D7A6E" }}>Book this service →</Link>
              </div>
            ))}
          </div>
          <div style={{ textAlign: "center" }}>
            <Link href="/book?category=attendant"
